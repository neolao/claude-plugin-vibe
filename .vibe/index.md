...
- Every agent lives in `agents/<name>.md` with `name` + `description`, in one of two families.
  ...
  - `expert-*`: one domain per agent, prescribes requirements before/during implementation — limited to domains without a `review-*` counterpart (see `decisions/001`), except `expert-realtime-rendering` (see `decisions/003`).
