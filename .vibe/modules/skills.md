...
- `/vibe:auto` (`skills/auto/SKILL.md`) — drain the backlog autonomously: ranks eligible items (unblock-count → fix-over-feature → lowest number, not lowest number alone), runs each one through `vibe:feature`/`vibe:fix` in `--auto` mode inside a dedicated sub-agent, persists and commits its state at every item boundary so any interruption is resumed on the next invocation; prints a compact `●`/`✓`/`⚠` status line per item
...
- `/vibe:next-task` (`skills/next-task/SKILL.md`) — pick the next eligible backlog item across every `active` repo in a workspace (or within the current repo alone if no workspace is detected), hand it to `vibe:feature`/`vibe:fix` (directly, or repeatedly with `--auto` in auto mode, never delegated to `vibe:auto`'s own selection), then push and — if the changelog warrants it — release (self-heals one retry if a pre-existing tracked test failure blocks it); the only skill in the plugin that pushes/publishes on its own
- `vibe:tasks` (`skills/tasks/SKILL.md`) — internal, `user-invocable: false`: creates a task list for the invoking skill via `TaskCreate`, or falls back to a scratchpad checklist if that tool is unavailable; invoked by `init`, `feature`, `fix`, `review`, `docs`, `release`, and `workspace-init`, never directly by users; also owns the compact `●`/`✓` chat status-line convention used on every task transition (reused per-item by `auto`/`next-task`)
