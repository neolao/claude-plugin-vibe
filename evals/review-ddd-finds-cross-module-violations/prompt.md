---
name: review-ddd-finds-cross-module-violations
description: review-ddd must find one violation per category across the modules of fixtures/shop/, each only visible when files are read together, without flagging a read of an aggregate's read-only view
tags: [review-ddd, recall]
runs: 3
max_turns: 10
timeout_seconds: 300
...
Invoke the `vibe:review-ddd` sub-agent (Agent tool, `subagent_type: "vibe:review-ddd"`) to review `fixtures/` in this case directory. Give it exactly this prompt:
