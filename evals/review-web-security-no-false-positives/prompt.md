---
name: review-web-security-no-false-positives
description: review-web-security must not flag the containment-checked file serving, escaped search output, sanitized header write, admin auth check, allowlisted SSRF fetch, capped report, hardened session cookie, generic error message, or the present security headers
tags: [review-web-security, precision]
runs: 3
max_turns: 10
timeout_seconds: 300
...
Scope: fixtures/
Dynamic verification: not enabled — static review only, do not launch or probe any server.
