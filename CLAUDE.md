...
- **`add_dirs` ne nomme que des dossiers de fixtures du cas** : `add_dirs: [.]` est refusé par le CLI (« an add_dir may only name fixture directories inside this case ») et chaque run échoue à $0 — écrire `add_dirs: [fixtures]`. Plusieurs cas de skills portent encore `[.]` : le corriger en premier au moment d'évaluer leur cible (vu sur changelog le 2026-09-24, puis sur auto le 2026-09-26).
...
- **`--allow-tools` only takes gated tools** (`Bash`, `Write`, `Edit`, `WebFetch`, `mcp__*`), space-separated (`--allow-tools Bash Write`), not the case's `allowed_tools` joined with commas: `Read`/`Grep`/`Glob`/`Agent` come from `case.yaml`.
- **The eval sandbox has no network.** A tool that queries a registry (`npm audit`, `pip-audit`…) fails ("denied access to registry.npmjs.org", seen on `review-dependencies` on 2026-09-25) or, with `--offline`, returns "0 vulnerabilities" without checking anything. A case that depends on an audit therefore measures how the agent handles an audit that could not run, never the tool's real output.
- **Reading a Docker run's output**: the `--keep-temp` dirs die with the container (`--rm`), but each run's last message stays in `evals/results/<timestamp>-<case>/aggregate-result.json`, in the `cases[0].arms.with[<run>].graders[<i>].evidence` field of the `llm` graders with `focus: last_message`. Graders only keep their votes there, not the judge's reasoning.
