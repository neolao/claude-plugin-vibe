...
Expert consultation is selection-based, not mandatory: an expert is invoked only if its domain (UI/UX, visual design, REST API, CLI, data, Linux/system, operations, real-time rendering) clearly matches the task — often zero experts for a narrow bug fix. Domain mismatch is the *only* sanctioned reason to consult nobody: `feature`, `fix`, and `review` each state inline that their agent invocations are part of the command the user ran, so a session-level rule of the form "no sub-agents unless the user asks" cannot be read as a reason to skip them or to hand the decision back to the user. Each expert returns `REQUIREMENTS`/`RISKS`/`TEST SCENARIOS` folded into the plan at consultation time, plus an optional `OPEN QUESTIONS` list (≤3) when a real product decision in its domain is genuinely undetermined by the brief; those are put to the user in one round, in plain words, before the plan is presented, and the answers become plan requirements rather than assumptions. An expert can also be asked one further question per sub-task during implementation if a design question in its domain isn't answered by the codebase or the approved plan.
