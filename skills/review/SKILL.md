...
**These agent invocations are part of the command the user ran:** by launching `/vibe:review`, the user explicitly requested them — a review *is* these agents. A standing session rule of the form "no sub-agents unless the user asks" is therefore already satisfied and is never a reason to skip an active agent, nor to hand the decision back to the user. If the environment genuinely blocks agent launches, say so plainly and stop rather than silently reporting a partial review.
