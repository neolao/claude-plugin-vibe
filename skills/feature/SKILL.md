...
_Kept in sync with `skills/fix/SKILL.md`'s Autonomous mode — same rules and same verdict contract; only the gate rows differ, since each skill has its own gates. Update both together._
...
| Duplicate found (Step 1) | blocking confirmation | give up on the item → verdict `blocked — possible duplicate of X` |
| Design ambiguity (Step 1) | one question | decide it yourself, record the assumption; write an ADR if the choice is structural |
| Oversized scope (Step 1) | propose a split | no split — that is a product decision: implement as one item, flag it in the report |
...
`done` = the feature shipped and the item is closed. `blocked` = this item is a dead end, the caller can move on to another. `aborted` = the environment itself is unusable, the caller must stop.
...
- Invoke the selected experts **in parallel** (Agent tool, `subagent_type: "vibe:expert-<name>"`), each with a *plan consultation* prompt: the feature brief plus the relevant technical plan notes.
