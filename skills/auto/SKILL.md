...
- The completion notification is the primary wake signal — you are re-invoked automatically when the sub-agent finishes, no polling needed. As a safety net for the case it hangs or the notification never arrives, schedule one long fallback wakeup right after launching (`ScheduleWakeup`, 1200–1800s) rather than a short polling interval; if it fires before the agent is actually done, just reschedule further out instead of blocking on a foreground wait.
- Either way the verdict arrives — the notification firing on its own, or this fallback wakeup — **that turn is not a direct user message**, so its result sits silently in context until the user happens to look. Use `PushNotification` to actually surface it once picked up: a report no one is there to read is not a report.
