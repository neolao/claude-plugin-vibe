...
<!-- vibe:begin:features -->
- Automated, TDD-first workflow: implement features and fix bugs with tests written first, in the human's stead, actively checked against writing tests that can't actually fail (tautological tests); work in progress is always committed before ending a turn, however the conversation unfolds, so nothing is lost across a session reset
