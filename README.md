...
- An autonomous mode that works the backlog on its own: eligible items are picked one after another and shipped without a single question or approval step, each one handled in isolation so a long run stays affordable; progress is committed at every item boundary, so an interruption — a closed session, a crash, a usage limit reached mid-item — is picked back up exactly where it stopped on the next run, and an item that turns out to be a dead end is set aside with its reason instead of stalling everything else. It surfaces a notification as soon as an item's result is ready during an unattended run instead of leaving it sitting unseen, and can also be told to process just one item at a time — useful for pacing work out for human review, cost control, or CI capacity — instead of always draining the whole backlog in one go
...
- Every workflow keeps tracking its progress and announces it clearly even in environments where the dedicated task system isn't available, instead of surfacing a confusing error — each step or item shows up in chat as a compact one-line marker instead of a wall of prose, so a long run stays skimmable
